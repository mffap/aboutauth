---
sidebar_label: Endpoints and Requests
tags: [oidc]
---
...
Termination | [Back-channel Logout](#back-channel-logout) | (POST) `/backchannel_logout` | End user session
...
The client application redirects the user to the `/authorize` to this endpoint with an [**Authentication Request**](3-authentication-request.md) that sends all required information as URL parameters.
...
You can call the token endpoint with a [**Token Request**](./4-token-request.md) during an authentication flow, or the Refresh Token Request when you have a refresh token and want to receive a new access token without re-authenticating.
...
After receiving an authorization code, the client sends a [**Token Request**](./4-token-request.md) to the Token Endpoint to obtain the tokens.
